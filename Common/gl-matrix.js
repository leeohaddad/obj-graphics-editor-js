...
(function (root, factory) {
     ...
     * @param {matrix4} proj Projection matrix
    ...
    vec3.unproject = function (vec, view, proj, viewport, dest) {
        ...
        matrix4.multiply(proj, view, m);
        if(!matrix4.inverse(m)) { return null; }
        
        matrix4.multiplyVec4(m, v);
    ...
    /**
     * Copies the elements of a mat3 into the upper 3x3 elements of a matrix4
     *
     * @param {mat3} mat mat3 containing values to copy
     * @param {matrix4} [dest] matrix4 receiving copied values
     *
     * @returns {matrix4} dest if specified, a new matrix4 otherwise
     */
    mat3.tomatrix4 = function (mat, dest) {
        if (!dest) { dest = matrix4.create(); }
     ...
     * Any javascript array-like object containing at least 16 numeric elements can serve as a matrix4
     *
     * @param {matrix4} [mat] matrix4 containing values to initialize with
     *
     * @returns {matrix4} New matrix4
     */
    matrix4.create = function (mat) {
     ...
     *
     * @param {matrix4} mat matrix4 containing values to copy
     * @param {matrix4} dest matrix4 receiving copied values
     *
     * @returns {matrix4} dest
     */
    matrix4.set = function (mat, dest) {
        dest[0] = mat[0];
        dest[1] = mat[1];
        dest[2] = mat[2];
        dest[3] = mat[3];
     ...
     *
     * @param {matrix4} dest matrix4 to set
     *
     * @returns {matrix4} dest
     */
     ...
     *
     * @param {matrix4} mat matrix4 to transpose
     * @param {matrix4} [dest] matrix4 receiving transposed values. If not specified result is written to mat
     *
     * @param {matrix4} dest is specified, mat otherwise
     */
    matrix4.transpose = function (mat, dest) {
        // If we are transposing ourselves we can skip a few steps but have to cache some values
        if (!dest || mat === dest) {
     ...
     *
     * @param {matrix4} mat matrix4 to calculate determinant of
     *
     * @returns {number} determinant of mat
     */
     ...
     *
     * @param {matrix4} mat matrix4 to calculate inverse of
     * @param {matrix4} [dest] matrix4 receiving inverse matrix. If not specified result is written to mat
     *
     * @param {matrix4} dest is specified, mat otherwise, null if matrix cannot be inverted
     */
     ...
     * @param {matrix4} [dest] matrix4 receiving copied values
     *
     * @returns {matrix4} dest is specified, a new matrix4 otherwise
     */
    matrix4.toRotationMat = function (mat, dest) {
        if (!dest) { dest = matrix4.create(); }
    ...
    /**
     * Copies the upper 3x3 elements of a matrix4 into a mat3
     *
     * @param {matrix4} mat matrix4 containing values to copy
     * @param {mat3} [dest] mat3 receiving copied values
     *
     ...
     */
    matrix4.toMat3 = function (mat, dest) {
        if (!dest) { dest = mat3.create(); }
    ...
    /**
     * Calculates the inverse of the upper 3x3 elements of a matrix4 and copies the result into a mat3
     * The resulting matrix is useful for calculating transformed normals
     *
     * Params:
     * @param {matrix4} mat matrix4 containing values to invert and copy
     * @param {mat3} [dest] mat3 receiving values
     *
     * @returns {mat3} dest is specified, a new mat3 otherwise, null if the matrix cannot be inverted
     */
    matrix4.toInverseMat3 = function (mat, dest) {
        // Cache the matrix values (makes for huge speed increases!)
        var a00 = mat[0], a01 = mat[1], a02 = mat[2],
     ...
     * Performs a matrix multiplication
     *
     * @param {matrix4} mat First operand
     * @param {matrix4} mat2 Second operand
     * @param {matrix4} [dest] matrix4 receiving operation result. If not specified result is written to mat
     *
     * @returns {matrix4} dest if specified, mat otherwise
     */
    matrix4.multiply = function (mat, mat2, dest) {
        if (!dest) { dest = mat; }
     ...
     *
     * @param {matrix4} mat matrix4 to transform the vector with
     * @param {vec3} vec vec3 to transform
     * @param {vec3} [dest] vec3 receiving operation result. If not specified result is written to vec
     *
     * @returns {vec3} dest if specified, vec otherwise
     */
    matrix4.multiplyVec3 = function (mat, vec, dest) {
        if (!dest) { dest = vec; }
     ...
     */
    matrix4.multiplyVec4 = function (mat, vec, dest) {
        if (!dest) { dest = vec; }
     ...
     *
     * @param {matrix4} mat matrix4 to translate
     ...
     *
     * @param {matrix4} mat matrix4 to scale
     * @param {vec3} vec vec3 specifying the scale for each axis
     * @param {matrix4} [dest] matrix4 receiving operation result. If not specified result is written to mat
     *
     * @param {matrix4} dest if specified, mat otherwise
     */
     ...
     *
     * @param {matrix4} mat matrix4 to rotate
     ...
     * @returns {matrix4} dest if specified, mat otherwise
     */
    matrix4.rotateX = function (mat, angle, dest) {
     ...
     * @returns {matrix4} dest if specified, mat otherwise
     */
    matrix4.rotateY = function (mat, angle, dest) {
        var s = Math.sin(angle),
            c = Math.cos(angle),
            a00 = mat[0],
            a01 = mat[1],
     ...
     * @returns {matrix4} dest if specified, mat otherwise
     */
    matrix4.rotateZ = function (mat, angle, dest) {
        var s = Math.sin(angle),
            c = Math.cos(angle),
            a00 = mat[0],
            a01 = mat[1],
     ...
     * @param {matrix4} [dest] matrix4 frustum matrix will be written into
     *
     * @returns {matrix4} dest if specified, a new matrix4 otherwise
     */
    matrix4.frustum = function (left, right, bottom, top, near, far, dest) {
        if (!dest) { dest = matrix4.create(); }
        var rl = (right - left),
            tb = (top - bottom),
            fn = (far - near);
     ...
     *     var quatMat = matrix4.create();
     *     quat4.tomatrix4(quat, quatMat);
     *     matrix4.multiply(dest, quatMat);
     *
     * @param {quat4} quat Rotation quaternion
     * @param {vec3} vec Translation vector
     * @param {matrix4} [dest] matrix4 receiving operation result. If not specified result is written to a new matrix4
     *
     * @returns {matrix4} dest if specified, a new matrix4 otherwise
     */
    matrix4.fromRotationTranslation = function (quat, vec, dest) {
        if (!dest) { dest = matrix4.create(); }
     ...
     *
     * @param {matrix4} mat matrix4 to represent as a string
     *
     * @returns {string} String representation of mat
     */
    matrix4.str = function (mat) {
     ...
     *
     * @param {quat4} quat quat4 to create matrix from
     * @param {matrix4} [dest] matrix4 receiving operation result
     *
     * @returns {matrix4} dest if specified, a new matrix4 otherwise
     */
    quat4.tomatrix4 = function (quat, dest) {
        if (!dest) { dest = matrix4.create(); }
